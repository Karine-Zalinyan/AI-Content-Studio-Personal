...
form.addEventListener('submit',async e=>{e.preventDefault();button.disabled=true;status.className='status';status.textContent='Starting generation…';exportLink.style.display='none';try{const labels={transformation:'Cheap to Luxury Food Transformation',ingredient:'One Ingredient to Multiple Recipes',story:'Food Story',satisfying:'Satisfying Cooking Transformation'};const body=new URLSearchParams({topic:labels[format.value]+': '+topic.value});const r=await fetch('/generate',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body});const d=await r.json();if(!r.ok)throw new Error(d.error||'Unable to start generation');poll(d.job_id)}catch(err){status.className='status error';status.textContent=err.message;button.disabled=false}});
